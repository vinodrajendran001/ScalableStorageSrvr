...
public class KVActions implements KVActionsInterface {
	...
	@Override
	public void initService(int numberOfNodes, int port) throws Exception {
		String content = "ssh -n 127.0.0.1 nohup java -jar /Users/roshinisachithanandan/git/ScalableStorageSrvr/ScalableStorageService/ms3-server.jar"
				+ port + " ERROR &";
		String script = "/Users/roshinisachithanandan/git/ScalableStorageSrvr/ScalableStorageService/src/app_kvEcs/script.sh";
			...
			do {
				...
				for (int i = 0; i < p.size(); i++) {
					if ((!p.get(i).isAccepting()) && (randomInt == i)) {
						...
						p.get(i).setAccepting(true);
						p.get(i).setPosition(
								hash.hashIpPort(p.get(i).getIp(), p.get(i)
										.getPort()));
						/*
						 * Initiate server with meta-data and start it using the
						 * correct command
						 */
						/* Update metadata */
